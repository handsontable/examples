...
| `workers/api/` | Orchestration + sharing worker `handsontable-demos-api` (sessions, `/api/demos`, `/d`, `/embed`, build snapshotter, `/api/chat`, and `/api/mcp/demos` + `PATCH /api/mcp/demos/:id` — headless create/update for the Handsontable MCP, ADR-0033). |
