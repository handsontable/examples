...
| `workers/api/` | Orchestration + sharing worker `handsontable-demos-api` (sessions, `/api/demos`, `/d`, `/embed`, build snapshotter, `/api/chat`, and `/api/mcp/demos` + `PATCH /api/mcp/demos/:id` + `GET /api/mcp/demos/:id/status` — headless create/update for the Handsontable MCP, ADR-0033; cold tier-2 builds run detached via the `BuildJob` DO, ADR-0039). |
...
- **Cost guardrails** (DEV-2030, ADR-0022): `max_instances` 10/5 is the container cap **and** the live-preview concurrency limit (no queue sits in front of the pool; the overflow caller gets the `at_capacity` 503) — raised from 5/3 in DEV-2909, and not to be changed again without redoing the arithmetic in `docs/cost-guardrails.md` in the same commit. Spend degrades live sessions in stages while static shares keep serving. The dollar thresholds and the enforcement switch are **editable at runtime in `/admin`** (stored in `runner_settings`); the `BUDGET_*` vars are only defaults. Pool pressure — `at_capacity` refusals, hourly awake-seconds and sampled peak concurrency — was specified by **ADR-0040** and is delivered under **ADR-0041** (not yet built): the `at_capacity` counter in `usage_daily` as ADR-0040 C.1 says, awake-seconds per hour and peak concurrency as Analytics Engine points, because daily totals cannot yield peak concurrency and that is what sizing the pool actually depends on.
- **Observability** (ADR-0041, **proposed, not yet built**): the plan is Sentry for uncaught errors (errors escaping a handler, ADR-0041 §E.1) and spend alerts, and everything else in a self-hosted, sleeping Loki + Grafana box (`workers/o11y/`, `containers/o11y/`) fed over OTLP, with metrics in Workers Analytics Engine. Until it ships, Sentry remains the fault channel as `wrangler.jsonc` says. New telemetry written from now on should already follow the ADR's `hot.*` attribute set and its "never a label" rule for demo id, session id, cf-ray and the user pseudonym. ADR-0042 (example analytics) and ADR-0043 (`/admin` cutover) follow it. Shared names, Analytics Engine slots and payload shapes are frozen in [`docs/observability-contract.md`](docs/observability-contract.md); on the `feat/runner-observability` branch the implementation tasks live in `tasks/o11y/` (temporary, deleted before merge).
...
- **A starter states its colour scheme; the shell drives the ones that don't** (DEV-2561, `docs/adr/0035-*`). `mainTheme` carries no `colorScheme`, so a bare `theme: mainTheme` renders whatever the *visitor's OS* prefers — which is why every starter passes `{ ...mainTheme, colorScheme: 'light' }`. A demo with no declaration of its own follows the shell's toggle over the `hot-runner-scheme` bridge (`packages/runtime/src/scheme.ts`), injected into the preview document on both tiers and never into the authored file map. Whether the shell may decide is settled in `App.tsx`, not in the receiver: a docs example or a Style-panel-wired demo owns its own scheme and is sent `auto`.
