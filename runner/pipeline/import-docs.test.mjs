...
import { importDocs, writeDocsBucketIndex } from "./import-docs.mjs";
...
function makeFixture(t, version = "18.0.3") {
  ...
  // A tmp-rooted indexPath, always passed explicitly: importDocs's own default
  // (runner/docs-buckets.json) is the real committed file, and these fixtures
  // must never write test bucket names into it.
  return {
    docsDir,
    outDir: path.join(root, "docs-examples"),
    indexPath: path.join(root, "docs-buckets.json"),
  };
}

test("regenerating a release bucket preserves a sibling next bucket", async (t) => {
  const { docsDir, outDir, indexPath } = makeFixture(t);
  ...
  await importDocs({ docsDir, docsBranch: "prod-docs/18.0", outDir, indexPath });
...
// DEMOS-1C: the client must not discover a by-design-absent bucket by
// fetching and failing (ADR-0021 #2/#3). It needs a static index of what is
// actually on disk. ADR-0021 #5 scopes each import's wipe to one bucket
// subtree, so the index has to be rederived from the whole directory listing
// every run — never accumulated — or a bucket regenerated by a different
// matrix leg would silently drop out of it.
test("importDocs rewrites the bucket index from the full directory listing, not just the imported bucket", async (t) => {
  const { docsDir, outDir, indexPath } = makeFixture(t);
  // A bucket that already existed on disk before this run, from an earlier
  // matrix leg / earlier day's import — the run below only touches "18.0".
  const priorDir = path.join(outDir, "17.1");
  fs.mkdirSync(priorDir, { recursive: true });
  fs.writeFileSync(path.join(priorDir, "manifest.json"), '{"bucket":"17.1"}\n');

  await importDocs({ docsDir, docsBranch: "prod-docs/18.0", outDir, indexPath });

  const index = JSON.parse(fs.readFileSync(indexPath, "utf8"));
  assert.deepEqual(index.buckets, ["17.1", "18.0"]);
});
...
test("fails and skips the artifact when generated files miss the module entry", async (t) => {
  ...
  await assert.rejects(
    importDocs({ docsDir, docsBranch: "prod-docs/18.0", outDir, indexPath }),
