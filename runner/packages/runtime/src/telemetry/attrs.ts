...
/**
 * T02-D4's AE-only browser attribute channel, ADR-0042's three columns
 * (T12). `workers/o11y/src/normalise/browser-attrs.ts#readAeOnlyAttrs` reads
 * `HotAttrs` fields `toAePoint` (§4/§5) accepts but §3 gives no
 * resource-attribute or structured-metadata slot to — `kind` (reserved as
 * `hot.metric_kind`, since `hot.kind` is already the Faro item kind, §3),
 * `ref`, `area` — under a `hot.<column>` key in a Faro item's raw
 * context/attributes, from the wire body, BEFORE `scrubTelemetry` runs
 * server-side. Mirrors the T07 fix round's `bucket`/`reason`/`fingerprint`
 * entries in this same category (merge target: one shared array once both
 * land) — see that fix round's doc comment for the full shape of this
 * mechanism.
 *
 ...
 * `STRUCTURED_METADATA_KEYS` entry either: `convert.ts#hoistAttributes` only
 * recognises `RESOURCE_ATTR_KEYS` and `STRUCTURED_KEY_SET`, so a key listed
 * here is silently dropped there and never reaches a stored
 ...
 *
 * Not part of contract §3's prose, same reasoning as the T07 fix round's
 * entries: `kind`/`ref`/`area` are §4/§5 (AE layout / metric registry)
 * concepts `HotAttrs` already names, gaining a transport path here, not a
 * new resource attribute or Loki label.
 */
export const ATTR_HOT_METRIC_KIND = "hot.metric_kind";
export const ATTR_HOT_REF = "hot.ref";
export const ATTR_HOT_AREA = "hot.area";

export const AE_ONLY_ATTRIBUTE_KEYS = [ATTR_HOT_METRIC_KIND, ATTR_HOT_REF, ATTR_HOT_AREA] as const;
