 ...
 * ruling). `workers/o11y/src/normalise/browser-attrs.ts#readAeOnlyAttrs`
 * reads `HotAttrs` fields `toAePoint` (§4/§5) accepts but §3 gives no
 * resource-attribute or structured-metadata slot to — `bucket`, `reason`,
 * `fingerprint`, and (T02-D4's list, not yet emitted by any browser call
 * site) `route_class`, `model`, `provider`, `device`, `ref`, `area` — under a
 * `hot.<column>` key in a Faro item's raw context/attributes, from the wire
 * body, BEFORE `scrubTelemetry` runs server-side.
 *
 ...
 * `resourceAttributes`/`attributes` field. Only `readAeOnlyAttrs`, reading
 * the raw wire body directly (before `hoistAttributes` ever runs), sees it.
 *
 * Scoped to this fix round's three fields only — `sandpack.compile_error`
 * (`fingerprint`), `version.switch` (`reason`), `preview.ready_ms`/
 * `bucket.resolve_ms` (`bucket`). The other six of T02-D4's nine AE-only
 * columns have no browser emitter yet (`route_class`/`model`/`provider` are
 * API-worker-only metrics today; `device`/`ref`/`area` are ADR-0042/T09's),
 * so adding them here now would be untested, dead allowlist surface — left
 * for whichever task adds the first browser call site that needs one.
 *
 ...
 * case): the controller's ruling on this fix round is to edit the doc only
 * when a DOCUMENTED name changes, and none of these three names was
 * previously documented anywhere in §3 — they are §4/§5 (AE layout / metric
 * registry) concepts that `HotAttrs` already names, gaining a transport path
 * here, not a new resource attribute or Loki label the doc's attribute table
 * would need to grow.
 */
