...
/** Bounds one prune call's cost — see `ledger.ts#PRUNE_BATCH_LIMIT`'s doc
 *  comment for the same reasoning. */
const FP_PRUNE_BATCH_LIMIT = 500;
...
const FP_PRUNE_SCAN_LIMIT = 2000;

/** Returns `fp:<fp>` → `nowMs` entries for every fingerprint in
