...
// The bucket DEMOS-1C suggests to a visitor on an absent version: whatever the
// generated index holds, resolved with the app's own helper rather than restated
// here. `docs-buckets.json` is regenerated by every docs import, so a hard-coded
// minor goes stale the next time one lands (18.0 -> 18.1 is what broke it).
const DOCS_BUCKETS: string[] = JSON.parse(
  readFileSync(new URL("../docs-buckets.json", import.meta.url), "utf8"),
).buckets;
const SUGGESTED_BUCKET = highestReleaseBucket(DOCS_BUCKETS);
if (!SUGGESTED_BUCKET) throw new Error("docs-buckets.json holds no release bucket to suggest");
const SUGGESTION = new RegExp(`available for Handsontable ${SUGGESTED_BUCKET.replace(".", "\\.")}`);
...
test("an open docs example with no target bucket stops preview and remains recoverable", async ({ page }) => {
  ...
  // literal message named only the selected version and never the suggestion.
  await expect(
    page
      .getByText(
        new RegExp(`No documentation examples are available for Handsontable 17\\.1\\.0.*${SUGGESTION.source}`),
      )
      .first(),
  ).toBeVisible();
