...
const angularDist = path.resolve(__dirname, '../client-angular/dist');
const reactDist = path.resolve(__dirname, '../client-react/dist');
...
//   GET /angular.html      → client-angular/dist/browser/angular.html
//   GET /angular-assets/*  → client-angular/dist/browser/*
const angularPlugin = {
  name: 'serve-angular',
  enforce: 'pre',
  configureServer(server) {
    server.middlewares.use('/angular.html', (_req, res) => {
      ...
      if (fs.existsSync(file)) {
        ...
        res.statusCode = 503;
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.end('<p>Angular app not built yet. Run: <code>cd client-angular && npm run build</code></p>');
      }
    });
  ...
  configureServer(server) {
    server.middlewares.use('/react.html', (_req, res) => {
      ...
      if (fs.existsSync(file)) {
        ...
        res.statusCode = 503;
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.end('<p>React app not built yet. Run: <code>cd client-react && npm run build</code></p>');
      }
    });
