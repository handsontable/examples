...
/**
 * Seeds the database with 55 product rows on application startup.
 *
 * The guard `if (repository.count() == 0)` prevents duplicate inserts on
 * subsequent restarts when the PostgreSQL volume persists.
 */
@Configuration
