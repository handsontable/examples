...
app.get("/codesandbox-vm", async (req, res) => {
  ...
  try {
    ...
    let sandbox = await sdk.sandboxes.create({
      ...
      public: true,
      tags: tags,
      id: packageJson.config?.codesandbox?.templateId,
      privacy: "public",
    });
    ...
    if (slackWebhook) {
      fetch(slackWebhook, {
        ...
        body: JSON.stringify({ "type": "mrkdwn", "text": `Codesandbox Error: ${error.message}: Debug: ${JSON.stringify({ exampleDir, exampleBranch, handsontableVersion, handsontableBranch, handsontableSha })}` })
      });
    }
