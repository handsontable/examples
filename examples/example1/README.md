...
A general purpose demo showcasing some of the most popular features of Handsontable in TypeScript, built with Vite. Handsontable is imported modularly — `handsontable/base` plus `registerAllModules()` — so it doubles as a starting point for trimming the bundle down to the modules you actually use.
...
Build a production bundle with `pnpm build`, and preview it with `pnpm preview`. The build type-checks first, so a TypeScript error fails it.
