...
// the runtime version: on >= 17 build the custom Base Web theme with the
// themes API, on 15/16 fall back to the CSS-based horizon theme, which every
...
export async function buildHotThemeProps(): Promise<HotThemeProps> {
  if (HOT_MAJOR >= 17) {
    const { buildBaseWebTheme } = await import('./hotThemeModern');
    return { theme: buildBaseWebTheme() };
  }
