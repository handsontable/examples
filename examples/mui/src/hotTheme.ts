...
// the runtime version: on >= 17 build the custom MUI theme with the themes
...
export async function buildHotThemeProps(muiTheme: Theme): Promise<HotThemeProps> {
  if (HOT_MAJOR >= 17) {
    const { buildMuiTheme } = await import('./hotThemeModern');
    return { theme: buildMuiTheme(muiTheme) };
  }
